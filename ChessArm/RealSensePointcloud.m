classdef RealSensePointcloud < handle
    ...
    methods
        ...
        function obj = RealSensePointcloud()
            ...
            obj.profile = obj.pipe.start(cfg);          
        ...
        function AdjustBrightness(obj, bright)
            dev = obj.profile.get_device();
            sens = dev.query_sensors();
            sens{1}{2}.set_option(realsense.option.brightness, bright);
        end
        ...
        function MyImage = getRGBframe(obj,aver)
            ...
            for i = 1:aver
                ...
                data = color.get_data();
                ts = addsample(ts,'Data',data,'Time',now);
            end
            %(Color data arrives as [R, G, B, R, G, B, ...] vector)
            %MyImage = permute(reshape(data',[3,color.get_width(),color.get_height()]),[3 2 1]);
            tsmean = uint8(mean(ts));
            MyImage = permute(reshape(tsmean',[3,color.get_width(),color.get_height()]),[3 2 1]);
        end
        ...
        function [X,Y,Z] = getPointcloud(obj,aver, filter)
            ...
            for i = 1:aver
                ...
                if (depth.logical())
                    ...
                    FiltDepthMap = permute(reshape(FiltDepthMap, [1280, 720]),[2 1]);
